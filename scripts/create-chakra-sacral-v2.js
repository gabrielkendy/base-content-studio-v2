...
async function run() {
  ...
  // Primeiro, deletar conteúdos antigos criados erroneamente
  console.log('Limpando conteúdos antigos do Chakra Sacral...');
  await supabase
    .from('conteudos')
    .delete()
  ...
  for (let i = 0; i < posts.length; i++) {
    const post = posts[i];
    console.log(`\n[${i+1}/${posts.length}] Processando: ${post.titulo}`);
    ...
    const imageUrl = await uploadFile(filePath, `flexbyo/chakra-sacral-v2-${timestamp}-${i+1}.png`);
    ...
    // Criar conteúdo com status aguardando_aprovacao
    ...
    if (conteudoError) {
      console.log('  ❌ Erro criar conteúdo:', conteudoError.message);
      continue;
    }
    console.log('  ✓ Conteúdo criado ID:', conteudo.id);
    ...
    const { data: aprovacaoLink, error: linkError } = await supabase
    ...
    if (linkError) {
      console.log('  ❌ Erro criar link:', linkError.message);
      continue;
    }
    console.log('  ✓ Token de aprovação criado');
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log(`✅ ${results.length} posts criados com links de aprovação!`);
  console.log('═══════════════════════════════════════════════════════════');
}
